...
Concerning microcontrollers I have been a little "lazy" during the past two years but I tried to at least keep an eye on what was going on. For next year I plan to continue my adventures into microcontroller land. I am now selecting the target platform for the upcoming projects.
...
I myself do not need the flexibility of supporting microcontroller boards besides ESP-32 and I love the easy and elegant way of adding components to my projects using ESP-IDF so I prefer using it directly.
...
With this you can call 'get_esp32' whenever you need it. If you do not need to use other frameworks you can add the PATH permanently.


## Try out everything with an example project

Copy the prepared example into a new project and configure:
...
Please make sure to provide sufficient power to your esp32 otherwise it will constantly reboot. At first I had it connected directly to my laptop (which obviously does not provide enough power). Now it is connected via a USB-Hub and works fine.

![Screenshot hello_world example](/media/esp_aws_iot/screenshot-hello-world.png)


# Now the AWS cloud integration example
...
You want to use your esp32 device to talk to your personal AWS IoT account. This needs a few steps and usually you have to do it just once but it is a bit involved especially if you do it the first time. There is no way around it but you will find on the web many useful tutorials on how to manage AWS IoT security certificates.

Using the AWS Console create a AWS IoT certificate and download it into the ~/esp/esp32/iot-bme280/main/certs folder

Add a policy to the certificate (I called it "farm_devices_policy") to define/limit which resources your device can access (here you want to limit the blast-radius):
...
we are looking for "Verification: OK"
...
For testing / troubleshooting purposes I use the MQTT client of the AWS IoT console to subscribe to my topics.

Once sent the measurements show up instantly in the MQTT client. The following screenshot shows the messages from my esp32 device dropping in:

![Store messages in an Amazon S3 bucket](/media/esp_aws_iot/aws_iot_mqtt_client.png)


## Add an IoT rule to write the measurements to S3 
...
For the rule "farm_archive_sensor_data" I use an SQL statement to wildcard-query all farm devices: 

![Store messages in an Amazon S3 bucket](/media/esp_aws_iot/aws_iot_rule.png)

I also setup an action that is used when the rule is triggered. For that I choose the relevant S3 bucket, define the key (relative path to store the file containing the timestamp of the message), and a role grant access to the S3 bucket.
